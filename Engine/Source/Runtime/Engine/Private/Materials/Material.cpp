...
int32 FMaterialResource::CompilePropertyAndSetMaterialProperty(EMaterialProperty Property, FMaterialCompiler* Compiler, EShaderFrequency OverrideShaderFrequency, bool bUsePreviousFrameTime) const
{
	...
	UMaterialInterface* MaterialInterface = MaterialInstance ? static_cast<UMaterialInterface*>(MaterialInstance) : Material.Get();
...
{
	FMaterial::BuildShaderMapIdOverride(Args);
#if WITH_EDITOR
...
#if WITH_EDITORONLY_DATA
void FMaterialResource::GetStaticParameterSet(FStaticParameterSet& OutSet) const
{
	FMaterial::GetStaticParameterSet(OutSet);
...
public:
	...
	{
		Material->StartCacheUniformExpressions();
	}

	virtual void FinishCacheUniformExpressions() const override
	{
		Material->FinishCacheUniformExpressions();
	}
	...
	virtual const FMaterial* GetMaterialNoFallback(ERHIFeatureLevel::Type InFeatureLevel) const override
	{
		...
		const FMaterial* MaterialResource = Material->GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(InFeatureLevel));
	...
	{
		const FMaterialResource* MaterialResource = Material->GetMaterialResource(Context.Material.GetShaderPlatform());
		if (MaterialResource && MaterialResource->GetRenderingThreadShaderMap())
		{
			if (Type == EMaterialParameterType::Scalar && ParameterInfo.Name == SubsurfaceProfile::GetSubsurfaceProfileParameterName())
			{
				// When using Substrate with hidden conversion, this path can still be used. Since GetSubsurfaceProfileRT() won't be correct 
				// (it will retrieve the SSS profile override instead of the actual SSS Profile), we retrieve the SSS profile through the 
				// array version at index 0
				if (Substrate::IsSubstrateEnabled())
				{
					const USubsurfaceProfile* SSProfileOverrideRT = GetSubsurfaceProfileRT();
					const USubsurfaceProfile* SSProfileRT = NumSubsurfaceProfileRT() > 0 ? GetSubsurfaceProfileRT(0) : nullptr;
...
void UMaterialInterface::PrecacheDefaultMaterialPSOs()
{
	...
	if (RHISupportsManualVertexFetch(GMaxRHIShaderPlatform) || IsPSOShaderPreloadingEnabled())
	{
		for (int32 Domain = 0; Domain < MD_MAX; ++Domain)
		{
			if (GDefaultMaterials[Domain])
	...
	{
		if (GDefaultMaterials[MD_LightFunction])
		{
			if (PipelineStateCache::IsPSOPrecachingEnabled())
			{
				GDefaultMaterials[MD_LightFunction]->PrecachePSOs(AllVertexFactoryTypes, PrecachePSOParams, EPSOPrecachePriority::High, MaterialPrecacheRequestIDs);
			}
		}
	}
}
	...
	for (int32 ResourceIndex = 0; ResourceIndex < LoadedResources.Num(); ResourceIndex++)
	{
		...
		if (LoadedShaderMap && LoadedShaderMap->GetShaderPlatform() == GMaxRHIShaderPlatform)
		{
			...
			if (bIncludeShaderMap)
			{
				...
				FMaterialResource* CurrentResource = FindOrCreateMaterialResource(OutMaterialResourcesLoaded, OwnerMaterial, OwnerMaterialInstance, LoadedShaderMap->GetShaderPlatform(), LoadedQualityLevel);
...
{
	return FindMaterialResource(const_cast<TArray<FMaterialResource*>&>(MaterialResources), GetFeatureLevelShaderPlatform_Checked(InFeatureLevel), QualityLevel, bAllowDefaultQuality);
}
...
{
	return FindMaterialResource(const_cast<TArray<FMaterialResource*>&>(MaterialResources), InShaderPlatform, QualityLevel, bAllowDefaultQuality);
}
...
FMaterialResource* FindMaterialResource(TArray<FMaterialResource*>& MaterialResources, EShaderPlatform InShaderPlatform, EMaterialQualityLevel::Type QualityLevel, bool bAllowDefaultQuality)
{
	...
	for (int32 ResourceIndex = 0, NumMaterialResources = MaterialResources.Num(); ResourceIndex < NumMaterialResources; ++ResourceIndex)
	{
		...
		if (CurrentResource->GetShaderPlatform() == InShaderPlatform)
		{
...
{
	return FindOrCreateMaterialResource(MaterialResources, OwnerMaterial, OwnerMaterialInstance, GetFeatureLevelShaderPlatform_Checked(InFeatureLevel), InQualityLevel);
}
	...
	EMaterialQualityLevel::Type InQualityLevel)
{
	...
	if (InQualityLevel != EMaterialQualityLevel::Num)
	{
		...
		if (OwnerMaterialInstance)
		{
			OwnerMaterialInstance->GetQualityLevelUsage(QualityLevelsUsed, InShaderPlatform);
		}
		else
		{
			OwnerMaterial->GetQualityLevelUsage(QualityLevelsUsed, InShaderPlatform);
		}
	...
	FMaterialResource* CurrentResource = FindMaterialResource(MaterialResources, InShaderPlatform, QualityLevelForResource, false);
	if (!CurrentResource)
	{
		CurrentResource = OwnerMaterialInstance ? OwnerMaterialInstance->AllocatePermutationResource() : OwnerMaterial->AllocateResource();
		CurrentResource->SetMaterial(OwnerMaterial, OwnerMaterialInstance, InShaderPlatform, QualityLevelForResource);
...
void UMaterial::SetInitialValues()
{
	...
	bRootNodeOverridesDefaultDistortion_DEPRECATED = false;
}
...
void UMaterial::PostInitProperties()
{
	...
	{
		// CDO needs a deterministic guid for incremental cook of blueprints.
		StateId = FGuid();
	}
	else
	{
...
void UMaterial::GetUsedTextures(TArray<UTexture*>& OutTextures, TOptional<EMaterialQualityLevel::Type> InSpecificQualityLevel, TOptional<EShaderPlatform> InSpecificShaderPlatform) const
{
	OutTextures.Empty();
	...
	if (!FPlatformProperties::IsServerOnly())
	{
		...
		if (InSpecificQualityLevel.IsSet())
		{
			EMaterialQualityLevel::Type QualityLevel = InSpecificQualityLevel.GetValue();
			ensure(QualityLevel != EMaterialQualityLevel::Num);
			if (QualityLevel == EMaterialQualityLevel::Num)
			{
				QualityLevel = GetCurrentMaterialQualityLevelChecked();
			}
			QualityLevelRange = FInt32Range(QualityLevel, QualityLevel);
		}

#if WITH_EDITOR
		FInt32Range ShaderPlatformRange(0, EShaderPlatform::SP_NumPlatforms - 1);
		if (InSpecificShaderPlatform.IsSet())
		{
			EShaderPlatform ShaderPlatform = InSpecificShaderPlatform.GetValue();
			ensure(ShaderPlatform != EShaderPlatform::SP_NumPlatforms);
			if (ShaderPlatform == EShaderPlatform::SP_NumPlatforms)
			{
				ShaderPlatform = GetMaxShaderPlatformChecked();
			}
			ShaderPlatformRange = FInt32Range(ShaderPlatform, ShaderPlatform);
		}
#else
		FInt32Range ShaderPlatformRange(GMaxRHIShaderPlatform, GMaxRHIShaderPlatform);
#endif
		...
		{
			for (int32 ShaderPlatformIndex = ShaderPlatformRange.GetLowerBoundValue(); ShaderPlatformIndex <= ShaderPlatformRange.GetUpperBoundValue(); ++ShaderPlatformIndex)
			{
				if (!FDataDrivenShaderPlatformInfo::IsValid((EShaderPlatform)ShaderPlatformIndex))
				{
					continue;
				}

				const FMaterialResource* CurrentResource = FindMaterialResource(MaterialResources, (EShaderPlatform)ShaderPlatformIndex, (EMaterialQualityLevel::Type)QualityLevelIndex, true);
				if (CurrentResource)
				{
...
void UMaterial::GetUsedTexturesAndIndices(TArray<UTexture*>& OutTextures, TArray< TArray<int32> >& OutIndices, EMaterialQualityLevel::Type QualityLevel, EShaderPlatform InShaderPlatform) const
{
	...
	check(QualityLevel != EMaterialQualityLevel::Num && InShaderPlatform != EShaderPlatform::SP_NumPlatforms);

	if (!FPlatformProperties::IsServerOnly())
	{
		const FMaterialResource* CurrentResource = FindMaterialResource(MaterialResources, InShaderPlatform, QualityLevel, true);
		if (CurrentResource)
		{
...
void UMaterial::LogMaterialsAndTextures(FOutputDevice& Ar, int32 Indent) const
{
	...
	{
		const FMaterialResource* MaterialResource = FindMaterialResource(MaterialResources, GetFeatureLevelShaderPlatform_Checked(FeatureLevel), QualityLevel, false);
		if (MaterialResource)
		{
...
void UMaterial::OverrideTexture(const UTexture* InTextureToOverride, UTexture* OverrideTexture, ERHIFeatureLevel::Type InFeatureLevel)
{
	...
	{
		FMaterialResource* Resource = GetMaterialResource(GetFeatureLevelShaderPlatform_Checked(FeatureLevelsToUpdate[FeatureLevelIndex]));
		if (Resource)
		{
...
{
#if WITH_EDITOR
	FMaterialResource* Resource = GetMaterialResource(InShaderPlatform);
	if (Resource)
	{
		Resource->TransientOverrides.SetNumericOverride(Type, ParameterInfo, Value);
		if (ShouldRecacheMaterialExpressions(Resource, ParameterInfo))
		{
			RecacheUniformExpressions(false);
		}
	}
...
void UMaterial::ClearNumericParameterDefault(EMaterialParameterType Type, const FHashedMaterialParameterInfo& ParameterInfo, EShaderPlatform InShaderPlatform)
{
	...
	if (Resource)
	{
		Resource->TransientOverrides.ClearNumericOverride(Type, ParameterInfo);
		if (ShouldRecacheMaterialExpressions(Resource, ParameterInfo))
		{
			RecacheUniformExpressions(false);
		}
...
bool UMaterial::GetUsageByFlag(EMaterialUsage Usage) const
{
	...
	switch(Usage)
	{
		...
		case MATUSAGE_Voxels: UsageValue = bUsedWithVoxels; break;
...
{
	return IsCompilingOrHadCompileError(GetFeatureLevelShaderPlatform_Checked(InFeatureLevel));
}

bool UMaterial::IsCompilingOrHadCompileError(EShaderPlatform InShaderPlatform)
{
	const FMaterialResource* Res = GetMaterialResource(InShaderPlatform);
...
void UMaterial::SetUsageByFlag(EMaterialUsage Usage, bool NewValue)
{
	...
	switch(Usage)
	{
		...
		case MATUSAGE_Voxels:
		{
			bUsedWithVoxels = NewValue; break;
		}
...
FString UMaterial::GetUsageName(EMaterialUsage Usage) const
{
	...
	switch(Usage)
	{
		...
		case MATUSAGE_Voxels: UsageName = TEXT("bUsedWithVoxels"); break;
...
void UMaterialInterface::OverrideBlendableSettings(class FSceneView& View, float Weight) const
{
	...
	if(!DestNode)
	{
		...
		if(InitialMID)
		{
			...
			InitialMID->CopyScalarAndVectorParameters(*SourceData, View.GetShaderPlatform());
...
void UMaterial::CacheResourceShadersForRendering(bool bRegenerateId, EMaterialShaderPrecompileMode PrecompileMode)
{
	...
	if (FApp::CanEverRender())
	{
		...
		while (FeatureLevelsToCompile != 0)
		{
			...
			FMaterialResource* CurrentResource = FindOrCreateMaterialResource(MaterialResources, this, nullptr, ShaderPlatform, ActiveQualityLevel);
...
void UMaterial::GetNewResources(EShaderPlatform ShaderPlatform, TArray<FMaterialResource*>& NewResourcesToCache)
{
	...
	for (int32 QualityLevelIndex = 0; QualityLevelIndex < EMaterialQualityLevel::Num; QualityLevelIndex++)
	{
		...
		if (QualityLevelsUsed[QualityLevelIndex])
		{
			...
			NewResource->SetMaterial(this, nullptr, ShaderPlatform, (EMaterialQualityLevel::Type)QualityLevelIndex);
	...
	{
		FMaterialResource* NewResource = AllocateResource();
		NewResource->SetMaterial(this, nullptr, ShaderPlatform);
		NewResourcesToCache.Add(NewResource);
	}
}
...
void UMaterial::CacheShadersForResources(EShaderPlatform ShaderPlatform, const TArray<FMaterialResource*>& ResourcesToCache, EMaterialShaderPrecompileMode PrecompileMode, const ITargetPlatform* TargetPlatform)
{
	...
	for (int32 ResourceIndex = 0; ResourceIndex < ResourcesToCache.Num(); ResourceIndex++)
	{
		...
		const bool bSuccess = CurrentResource->CacheShaders(PrecompileMode, TargetPlatform);
...
void UMaterial::BeginCacheShadersForResources(EShaderPlatform ShaderPlatform, const TArray<FMaterialResource*>& ResourcesToCache, EMaterialShaderPrecompileMode PrecompileMode, const ITargetPlatform* TargetPlatform)
{
	...
	{
		FMaterialResource* CurrentResource = ResourcesToCache[ResourceIndex];
		CurrentResource->BeginCacheShaders(PrecompileMode, TargetPlatform,
...
void UMaterial::CacheGivenTypesForCooking(EShaderPlatform ShaderPlatform, ERHIFeatureLevel::Type FeatureLevel, EMaterialQualityLevel::Type QualityLevel, const TArray<const FVertexFactoryType*>& VFTypes, const TArray<const FShaderPipelineType*> PipelineTypes, const TArray<const FShaderType*>& ShaderTypes)
{
	...
	FMaterialResource* CurrentResource = FindOrCreateMaterialResource(MaterialResources, this, nullptr, ShaderPlatform, QualityLevel);
	check(CurrentResource);
...
bool UMaterial::IsComplete() const
{
	...
	if (FApp::CanEverRender())
	{
		...
		while (FeatureLevelsToCompile != 0)
		{
			...
			FMaterialResource* CurrentResource = FindMaterialResource(MaterialResources, ShaderPlatform, ActiveQualityLevel, true);
...
FGraphEventArray UMaterial::PrecachePSOs(const FPSOPrecacheVertexFactoryDataList& VertexFactoryDataList, const FPSOPrecacheParams& InPreCacheParams, EPSOPrecachePriority Priority, TArray<FMaterialPSOPrecacheRequestID>& OutMaterialPSORequestIDs)
{
	...
	if (FApp::CanEverRender() && MaterialResources.Num() > 0 && (PipelineStateCache::IsPSOPrecachingEnabled() || IsPSOShaderPreloadingEnabled()) && UMaterialInterface::IsDefaultMaterialInitialized())
	{
		...
		{
			const ERHIFeatureLevel::Type FeatureLevel = (ERHIFeatureLevel::Type)FBitSet::GetAndClearNextBit(FeatureLevelsToCompile);
			FMaterialResource* MaterialResource = FindMaterialResource(MaterialResources, GetFeatureLevelShaderPlatform_Checked(FeatureLevel), ActiveQualityLevel, true/*bAllowDefaultMaterial*/);
			if (MaterialResource)
			{
				GraphEvents.Append(MaterialResource->CollectPSOs(VertexFactoryDataList, InPreCacheParams, Priority, OutMaterialPSORequestIDs));
			}
		}
	}
	return GraphEvents;
...
FMaterialResource* UMaterial::GetMaterialResource(EShaderPlatform InShaderPlatform, EMaterialQualityLevel::Type QualityLevel)
{
	if (QualityLevel == EMaterialQualityLevel::Num)
	{
		QualityLevel = GetCachedScalabilityCVars().MaterialQualityLevel;
...
const FMaterialResource* UMaterial::GetMaterialResource(EShaderPlatform InShaderPlatform, EMaterialQualityLevel::Type QualityLevel) const
{
	if (QualityLevel == EMaterialQualityLevel::Num)
	{
		QualityLevel = GetCachedScalabilityCVars().MaterialQualityLevel;
...
bool UMaterial::ConvertMaterialToSubstrateMaterial(bool bAllowEmptyMaterialUpdate, bool bRequestedByUser)
{
	...
	*/
	...
	// * Store the current version of Substrate's auto-conversion
...
void UMaterial::PostLoad()
{
	...
	if (UE5MainVer < FUE5MainStreamObjectVersion::MaterialRefractionModeNone)
	{
		...
		// else keep the current refraction mode.
	}
#endif // WITH_EDITOR
	...
	{ 
		...
		if (IsUIMaterial() || IsDeferredDecal() || (MaterialDomain == MD_LightFunction) || IsPostProcessMaterial())
		{
			...
			PreloadMaterialShaderMap(GetMaterialResource(GMaxRHIShaderPlatform), Unused);
		}
	}
...
void UMaterial::SaveShaderStableKeysInner(const class ITargetPlatform* TP, const FStableShaderKeyAndValue& InSaveKeyVal)
{
	...
	if (MatRes)
	{
		for (const FMaterialResourceForCooking& Mat : *MatRes)
		{
			if (Mat.Resource)
			{
				Mat.Resource->SaveShaderStableKeys(SaveKeyVal);
			}
		}
	}
#endif
...
void UMaterial::CancelOutstandingCompilation()
{
	...
	{
		if (FMaterialResource* Res = GetMaterialResource((EShaderPlatform)ShaderPlatformIndex))
		{
													   ...
													   TFunction<UMaterialInterface*(const FString&)> ODSCCustomLoadMaterial)
{
	...
	for (const auto& Entry : CoalescedShadersToCompile)
	{
		...
		FMaterialResource* MaterialResource = MaterialInterface->GetMaterialResource(Shaders.ShaderPlatform, Shaders.QualityLevel);
		check(MaterialResource);
		check(MaterialResource->GetFeatureLevel() == Shaders.FeatureLevel);
		check(MaterialResource->GetShaderPlatform() == Shaders.ShaderPlatform);
...
bool UMaterial::SupportsShadingModelOverride() const
{
	...
	// That is when SubstrateShadingModels nodes are used. And only if hidden conversion is not enabled, otherwise we always display it.
	bool Out = true;
...
EBlendMode UMaterial::GetBlendMode() const
{
	...
	{
		if (GetShadingModels() == MSM_ThinTranslucent && !HasSubstrateFrontMaterialConnected() && Substrate::IsHiddenMaterialAssetConversionEnabled())
		{
			// Set the correct blend mode when using Substrate hidden conversion for thin translucent materials. Only when the FrontMaterial is not pluggedin.
			return BLEND_TranslucentColoredTransmittance;
		}
		return BlendMode;
	}
...
{
	return bEnableMobileSeparateTranslucency && !IsUIMaterial() && !IsDeferredDecal();
}
...
static bool IsPropertyActive_Internal(EMaterialProperty InProperty,
	...
	ERefractionMode RefractionMethod,
	...
	bool bFrontMaterialIsConnected)
{
	...
	if (bShowAsSubstrate)
	{
		...
		if (bIsSupported)
		{
			...
			{
			case MP_Refraction:
				Active = (bIsTranslucentBlendMode && !IsAlphaHoldoutBlendMode(BlendMode) && !IsModulateBlendMode(BlendMode) && bUsesDistortion && RefractionMethod != RM_IndexOfRefractionFromF0) || ShadingModels.HasShadingModel(MSM_SingleLayerWater);
				break;
			case MP_Opacity:
	...
	else
	{
		...
		case MP_ShadingModel:
			...
			break;
		case MP_SurfaceThickness:
			Active = false; // Only shown when material is Shown as Substrate (e.g., with FrontMaterial connected)
			break;
		case MP_FrontMaterial:
			{
...
uint32 UMaterial::ComputeAllStateCRC() const
{
	...
	// TODO:  Find a way to avoid invalidating StateID on changes that can't affect computation, like repositioning nodes in the material graph.
