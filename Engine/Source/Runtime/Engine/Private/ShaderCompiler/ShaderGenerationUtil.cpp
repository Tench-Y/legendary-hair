...
void ApplyFetchEnvironmentInternal(FShaderMaterialPropertyDefines& SrcDefines, const ContainerType& Container)
{
	...
	FETCH_COMPILE_BOOL(USES_FIRST_PERSON_INTERPOLATION);
	FETCH_COMPILE_BOOL(IS_MATERIAL_SHADER);

	FETCH_COMPILE_BOOL(SUBSTRATE_ENABLED);
	FETCH_COMPILE_INT(SUBSTRATE_GBUFFER_FORMAT);
...
static FShaderGlobalDefines FetchShaderGlobalDefines(EShaderPlatform TargetPlatform, EGBufferLayout GBufferLayout)
{
...

	{
		...
		Ret.CLEAR_COAT_BOTTOM_NORMAL = CVar ? (CVar->GetValueOnAnyThread() != 0 && (!bIsMobilePlatform || MobileSupportsSM5MaterialNodes(TargetPlatform))) : 0;
	}
...
static FString CreateGBufferDecodeFunctionDirect(const FGBufferInfo& BufferInfo)
{
	...
	FullStr += TEXT("\tif(bForceUnlitOrDefaultLit && Ret.ShadingModelID != 0) Ret.ShadingModelID = 1;\n");

	FullStr += TEXT("\tRet.WorldTangent = AnisotropicData.xyz;\n");
	FullStr += TEXT("\tRet.Anisotropy = AnisotropicData.w;\n");

	FullStr += TEXT("\tRet.NormalDistribution = DecodeNormal(SGGXData.xyz);\n");
...
static FString CreateGBufferDecodeFunctionVariation(const FGBufferInfo& BufferInfo, EGBufferDecodeType DecodeType, ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	FullStr += FString::Printf(TEXT("FGBufferData DecodeGBufferData%s(%s %s, bool bGetNormalizedNormal = true, bool bForceUnlitOrDefaultLit = false)\n"),
	...
	if (DecodeType == CoordUV)
	{
		...
		FullStr += TEXT("\tfloat4 SGGXData = Texture2DSampleLevel(SceneTexturesStruct.GBufferSGGXTexture, SceneTexturesStruct_SGGXTextureSampler, UV, 0).xyzw;\n");
	}
	else if (DecodeType == CoordUInt)
	{
		...
		FullStr += TEXT("\tfloat4 SGGXData = SceneTexturesStruct.GBufferSGGXTexture.Load(int3(PixelPos, 0)).xyzw;\n");
	}
	else if (DecodeType == SceneTextures)
	{
		...
		FullStr += TEXT("\tfloat4 SGGXData = GBufferSGGXTexture.SampleLevel(GBufferSGGXTextureSampler, UV, 0).xyzw;\n");
	}
	else if (DecodeType == SceneTexturesLoad)
	{
		...
		FullStr += TEXT("\tfloat4 SGGXData = GBufferSGGXTexture.Load(int3(PixelCoord, 0)).xyzw;\n");
	}
	else
	{
		check(0);
	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	bool bIsSubstrateNewGBuffer = Mat.SUBSTRATE_GBUFFER_FORMAT==1; // Similarly to FetchFullGBufferInfo, we do not check for MATERIAL_IS_SUBSTRATE as this is decided per project.
...
void FShaderCompileUtilities::ApplyDerivedDefines(FShaderCompilerEnvironment& OutEnvironment, FShaderCompilerEnvironment * SharedEnvironment, const EShaderPlatform Platform)
{
	...
	if (MaterialDefines.MATERIAL_IS_SUBSTRATE && MaterialDefines.SUBSTRATE_GBUFFER_FORMAT==1 && SubstrateFirstMRT > 0)
	{
