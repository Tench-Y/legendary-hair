...
enum ERefractionMode : int
{
	...
	/**
	 * Refraction is computed based on the camera vector entering a medium whose index of refraction is defined by the material IOR evaluated from F0.
	 * The new medium's surface is defined by the material's normal.  With this mode, a flat plane seen from the side will have a constant refraction offset.
	 * This is a physical model of refraction but causes reading outside the scene color texture so is a poor fit for large refractive surfaces like water.
	 */
	RM_IndexOfRefractionFromF0 UMETA(DisplayName = "Index Of Refraction From F0"),
};
...
/** 
* Enumerates available Substrate storage format
*/
UENUM()
namespace ESubstrateStorageFormat
{
	enum Type : int
	{
		/** Blendable GBuffer - Fast but with limited lighting complexity. If selected, all platforms will be forced to use this format. */
		BlendableGBuffer = 0 UMETA(DisplayName = "Blendable GBuffer"),

		/** Adaptive GBuffer - More heavy but enable complex lighting behavior per pixel. If selected, only compatible platforms will use this format. Uncompatible platforms will use Blendable GBuffer format. */
		AdaptiveBuffer = 1 UMETA(DisplayName = "Adaptive GBuffer"),
	};
}
...
namespace ESubstrateClosureConfig
{
	enum Type : int
	{
		/** Use the mininum between the platform's closure count and the project's closure count, on platforms supporting Adaptive GBuffer */
		UseMinProjectAndPlatforClosuresPerPixel = 0 UMETA(DisplayName = "Min of project's and platfom's closure count"),

		/** Force the project's closure count, on all platforms supporting Adaptive GBuffer */
		ForceProjectClosuresPerPixel = 1 UMETA(DisplayName = "Force Closure Count"),
	};
}
...
inline bool CollisionEnabledHasProbe(ECollisionEnabled::Type CollisionEnabled)
{
	return (CollisionEnabled == ECollisionEnabled::ProbeOnly) ||
			(CollisionEnabled == ECollisionEnabled::QueryAndProbe);
}

inline ECollisionEnabled::Type CollisionEnabledIntersection(ECollisionEnabled::Type CollisionEnabledA, ECollisionEnabled::Type CollisionEnabledB)
{
...
inline ECollisionEnabled::Type CollisionEnabledFromFlags(const bool bQuery, const bool bPhysics, const bool bProbe)
{
...
inline void CollisionEnabledToFlags(const ECollisionEnabled::Type CollisionEnabled, bool& bQuery, bool& bPhysics, bool& bProbe)
{
...
struct FRotationConversionCache
{
	...
	inline FQuat RotatorToQuat(const FRotator& InRotator) const
	{
	...
	inline FQuat RotatorToQuat_ReadOnly(const FRotator& InRotator) const
	{
	...
	inline FRotator QuatToRotator(const FQuat& InQuat) const
	{
		FPayload& Payload = GetOrCreatePayload();
	...
	inline FRotator QuatToRotator_ReadOnly(const FQuat& InQuat) const
	{
		if (LIKELY(PayloadPtr.IsValid()))
		{
			FPayload& Payload = *PayloadPtr;
	...
	inline FRotator NormalizedQuatToRotator(const FQuat& InNormalizedQuat) const
	{
		FPayload& Payload = GetOrCreatePayload();
	...
	inline FRotator NormalizedQuatToRotator_ReadOnly(const FQuat& InNormalizedQuat) const
	{
		if (LIKELY(PayloadPtr.IsValid()))
		{
			FPayload& Payload = *PayloadPtr;
	...
	inline FQuat GetCachedQuat() const
	{
	...
	inline FRotator GetCachedRotator() const
	{
		if (UNLIKELY(!PayloadPtr.IsValid()))
		{
...
inline ETeleportType TeleportFlagToEnum(bool bTeleport) { return bTeleport ? ETeleportType::TeleportPhysics : ETeleportType::None; }
inline bool TeleportEnumToFlag(ETeleportType Teleport) { return ETeleportType::TeleportPhysics == Teleport; }
...
struct FMeshNaniteSettings
{
	...
	bool operator==(const FMeshNaniteSettings& Other) const
	{
		...
		return bEnabled == Other.bEnabled
			...
			&& ShapePreservation == Other.ShapePreservation
...
public:
	...
	inline EWalkableSlopeBehavior GetWalkableSlopeBehavior() const
	{
		return WalkableSlopeBehavior;
	}

	/** Gets the slope angle used for the override behavior. */
	inline float GetWalkableSlopeAngle() const
	{
		return WalkableSlopeAngle;
	}

	/** Set the slope override behavior. */
	inline void SetWalkableSlopeBehavior(EWalkableSlopeBehavior NewSlopeBehavior)
	{
		WalkableSlopeBehavior = NewSlopeBehavior;
	}

	/** Set the slope angle used for the override behavior. */
	inline void SetWalkableSlopeAngle(float NewSlopeAngle)
	{
